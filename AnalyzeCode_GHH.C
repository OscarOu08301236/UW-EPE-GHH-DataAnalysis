...
void BookHistograms(ExRootResult *result, MyPlots *plots)
{
  ...
  plots->fAllJetPT = result->AddHist1D("jet_pt", "jet P_{T}",
                                         "jet P_{T}, GeV/c",
                                         "number of jet",
                                         50, 0.0, 8.0);
}
...
void AnalyseEvents(ExRootTreeReader *treeReader, MyPlots *plots)
{
  ...
  for(entry = 0; entry < allEntries; ++entry)
  {
    ...
    if(branchJet->GetEntriesFast() > 0)
    {
      ...
      //plots->fJetPT[0]->Fill(allJet->PT);
      //plots->histTest->Fill(allJet->PT);
      plots->fJetPT[1]->Fill(jet[0]->PT);
    }
    ...
    // Loop over all jet in event
    itJet.Reset();
    while((allJet = (TRootJet*) itJet.Next()))
    {
      plots->fAllJetPT->Fill(allJet->PT);
    }
  }
}
...
void AnalyzeCode_GHH(const char *inputFileList)
{
